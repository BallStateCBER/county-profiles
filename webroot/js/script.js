...
var countyProfiles = {
	...
	setupProfileSection: function (section) {
		...
		output_options.find('a').click(function (event) {
			...
			// Show table
				...
				countyProfiles.setupScrollingTableContainer(segment_name);
			}
		});
		
		// Set up subsegments
		if (section.hasClass('toggled_subsegments')) {
			this.setupSubsegments(section);
		}
	},
	...
	setupScrollingTableContainer: function (segment_name) {
		...
		if (overflow > 0) {
			...
			this.setupScrollingTableButton(table, container, segment_name, 'l');
			this.setupScrollingTableButton(table, container, segment_name, 'r');
		}
		
		scrolling_tables_set_up.push(segment_name);
	},
	
	setupScrollingTableButton: function (table, container, segment_name, direction) {
		if (direction == 'l') {
			var timeout_key = 0;
		} else {
			var timeout_key = 1;
		...
		$('#table_scroll_'+direction+'_'+segment_name).click(function(event) {
			event.preventDefault();
			countyProfiles.scrollContainer(table, container, direction, this);
		}).mousedown(function() {
			countyProfiles.scrollContainer(table, container, direction, this);
	...
	scrollContainer: function (table, container, direction, button) {
		var overflow = table.width() - container.width();
